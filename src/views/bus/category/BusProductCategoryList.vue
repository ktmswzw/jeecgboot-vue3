<template>
  <div>
    ...
    <a-row>
      ...
      <a-col :span="16">
        <bus-product-list> </bus-product-list>
      </a-col> </a-row
    ><!--字典弹窗-->
    <BusProductCategoryModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>

<script lang="ts" name="category-busProductCategory" setup>
  //ts语法
  import { ref, computed, unref, toRaw, nextTick, provide } from 'vue';
  ...
  const categoryId = computed(() => (unref(selectedRowKeys).length > 0 ? unref(selectedRowKeys)[0] : ''));
  //下发 mainId,子组件接收
  ...
  async function handleDelete(record) {
    await deleteBusProductCategory({ id: record.id }, importSuccess);
  }
  ...
  async function batchHandleDelete() {
    ...
    await batchDeleteBusProductCategory({ id: ids }, importSuccess);
  }
  /**
   * 导入
   */
